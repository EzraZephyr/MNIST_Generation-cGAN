...
from utils_en import discriminator
from utils_en import generator
from utils_en import dataloader
...
def train():
    ...
    with open(train_csv, 'w', newline='') as f:
        ...
        writer.writeheader()
        # Create a CSV file to record losses
    ...
    # Optimize discriminator and generator using Adam optimizer
    ...
    for epoch in range(num_epochs):
        ...
        for i, data in enumerate(train_loader, 0):
            ...
            # Create a tensor filled with 0.9 for label smoothing

            output_real = netD(real_images, real_labels)
            loss_d_real = criterion(output_real, real_targets)
            loss_d_real.backward()
            # Calculate loss for real samples and backpropagate

            noise = torch.randn(batch_size, noise_dim, 1, 1, device=device)
            fake_labels = torch.randint(0, num_classes, (batch_size,), device=device)
            fake_images = netG(noise, fake_labels)
            # Generate fake samples using random noise and random labels through generator
            ...
            # Create a tensor filled with 0, pass generated fake samples to
            # discriminator, calculate loss, and backpropagate

            optimizer_D.step()

            real_targets.fill_(1.0)
            # The generator aims to fool the discriminator into believing fake samples are real,
            # so the label is set to 1

            output_fake = netD(fake_images, fake_labels)
            loss_g = criterion(output_fake, real_targets)
            loss_g.backward()
            optimizer_G.step()
            # Calculate generator loss, backpropagate, and update parameters
        ...
        with open(train_csv, 'a', newline='') as f:
            ...
            # Write the loss of the current epoch to file
